...
void doEvent(Int_t nEvents=-1, const Char_t *inputFile="test.list", const TString outputFile="test/test.root", const Bool_t debug = kFALSE)
{
	...
	miniTreeMaker->setMaxVtxZ(60.);
	miniTreeMaker->setMaxVzDiff(3.);
	miniTreeMaker->setMaxTrackEta(1.);
	miniTreeMaker->setMaxDca(1.);
	miniTreeMaker->setMinNHitsDedx(15);
	miniTreeMaker->setMinNHitsFit(20);
	miniTreeMaker->setMinNHitsFitRatio(0.52);
	miniTreeMaker->setMaxBeta2TOF(0.025);
